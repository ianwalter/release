...
async function getReleaseBody (start, end, isVersionZero) {
  start = isVersionZero ? undefined : start
  const result = await commits({ start, end })
...
async function release (config) {
  ...
  // TODO: comment
  if (!config.version) {
    Object.assign(config, await getVersion(config.packageJson))
  }

  // TODO: comment
  if (config.isVersionZero) {
    config.access = await getAccess(config.version)
  }
  ...
  // TODO: comment
  await commitVersionUpdate(config.version)
  ...
  print.debug('Release body:', releaseBody)
